...
'sample code
    ...
    Private Sub OnClick2(ByVal sender As Object, ByVal e As EventArgs)
		try           	
			...
			text0.text =text0.text+CHR(13)+CHR(10)+":FINISH"+CHR(13)+CHR(10)+"if sucess the exe name will be out.exe and asm name will be out.asm"
	...
	rtxt=file.readAlllines("./index.txt")
	ss=""
	s=""
	for each s in rtxt
		ss=ss+s+chr(10)
...
'general error
	...
	file.WriteAllText("out.asm",vvv)
		...
		try           	
			c= "-c 'timeout 59s nasm -o out.exe out.asm '"
			dim psi as ProcessStartInfo = new ProcessStartInfo()
			psi = new ProcessStartInfo()
			psi.FileName = "/bin/bash" 
			psi.CreateNoWindow=true
